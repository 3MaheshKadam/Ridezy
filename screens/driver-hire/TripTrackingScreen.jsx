...
const TripTrackingScreen = ({ navigation, route }) => {
  ...
  // Use assigned driver data from navigation params, fallback to mock data
  const driverData = tripDetails?.assignedDriver || {
    name: 'Rajesh Kumar',
    avatar: '👨‍💼', // Changed from 'photo' to match DriverMatching structure
    rating: 4.8,
    reviews: 142,
    phone: '+91 98765 43210',
    vehicleNumber: 'MH 27 AB 1234',
    vehicleModel: 'Honda City', // Changed from 'vehicleName' to match DriverMatching
    vehicleColor: 'White',
    experience: '5 years',
    completedTrips: 1248,
  };
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          >
            <View className="w-16 h-16 bg-accent/10 rounded-2xl justify-center items-center mr-4">
              <Text className="text-3xl">{driverData.avatar}</Text>
            </View>
      ...
      >
        <View className="flex-1 bg-black/50 justify-end">
          ...
          >
            ...
            <View className="items-center mb-6">
              <View className="w-24 h-24 bg-accent/10 rounded-3xl justify-center items-center mb-4">
                <Text className="text-5xl">{driverData.avatar}</Text>
              </View>
              <Text className="text-primary text-2xl font-bold mb-2">
                {driverData.name}
              </Text>
